...
```
Allí encontrarás las siguientes carpetas principales:
...
* jupyter-db/data: Contiene la definición de un contenedor de MySQL (almacena datos crudos y procesados) y un Dockerfile para levantar JupyterLab y correr el mlflow_notebook.ipynb.
...
* MySQL quedará disponible en puerto 3307.
...
* penguin.csv: Archivo raw que se utiliza para crear la base de datos en MySQL desde Jupyter.
...
docker compose up -d
```
* Crear el bucket mlflows3 en la consola de MinIO (puerto 9001) y la carpeta artifacts.
...
# 4. Levantar JupyterLab:
```
cd ../Jupyter
docker build -t jupyterlab .
docker run -it --name jupyterlab --rm -p 8888:8888 -v $PWD:/work jupyterlab:latest
```
* Dentro de Jupyter, abrir experiments.ipynb, cargar datos, entrenar, registrar runs en MLflow.
...
* Instancia de BD: MySQL para datos crudos y procesados (carpeta jupyter-db/data).
